...
public class GameScreen {
    ...
    public Scene createScene() {
        ...
        previewCanvas.setStyle("-fx-border-color: #00ff00; -fx-border-width: 1;");
    ...
    private void startGameLoop() {
        gameLoop = new AnimationTimer() {
            ...
            public void handle(long now) {
                ...
                // Chỉ update game khi không pause
                if (!isPaused) {
    ...
    private void updateUI(double deltaTime) {
        ...
        if (breakdown != null && breakdown.getTotalScore() > 0) {
            // Kiểm tra xem có breakdown mới không (so sánh với breakdown đã hiển thị trước đó)
        ...
        for (int i = 0; i < shape.length; i++) {
            for (int j = 0; j < shape[i].length; j++) {
                if (shape[i][j] == 1) {
                    ...
                    gc.fillRect(x + borderWidth, y + borderWidth, CELL_SIZE - borderWidth * 2, CELL_SIZE - borderWidth * 2);
                    ...
                    if (!settings.isHighContrastMode() && !settings.isReduceMotion()) {
                        ...
                        gc.setFill(lighter);
                        gc.fillRect(x + borderWidth, y + borderWidth, CELL_SIZE - borderWidth * 2, (CELL_SIZE - borderWidth * 2) / 3);
                        gc.setFill(color);
                    }
                }
            }
    ...
    private void drawPreview() {
        ...
        int offsetX = (int)((previewCanvas.getWidth() - shape[0].length * CELL_SIZE) / 2);
        int offsetY = (int)((previewCanvas.getHeight() - shape.length * CELL_SIZE) / 2);

        for (int i = 0; i < shape.length; i++) {
            for (int j = 0; j < shape[i].length; j++) {
                if (shape[i][j] == 1) {
                    gc.fillRect(offsetX + j * CELL_SIZE + 1, offsetY + i * CELL_SIZE + 1, 
    ...
    private void updateAnimations(double deltaTime) {
        // Check for new row clears
        game.ScoreBreakdown breakdown = gameController.getLastScoreBreakdown();
        if (breakdown != null && breakdown.getRowsCleared() > 0) {
            if (breakdown.getRowsCleared() != lastRowsCleared) {
                // New row clear - start animation
                isRowClearing = true;
                rowClearAnimationProgress = 0.0;
                lastRowsCleared = breakdown.getRowsCleared();
                
                // Create particle explosion (skip if reduce motion)
                GameSettings settings = GameSettings.getInstance();
                if (!settings.isReduceMotion()) {
                    ...
                    particleSystem.createRowClearExplosion(centerX, centerY, effectColor, 30);
        ...
        if (isRowClearing) {
            rowClearAnimationProgress += deltaTime * 2.0;
            if (rowClearAnimationProgress >= 1.0) {
                isRowClearing = false;
                rowClearAnimationProgress = 0.0;
            }
        }
